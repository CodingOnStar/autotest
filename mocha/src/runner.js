    ...
    if (suite._beforeAll.length) {
      for (const fn of suite._beforeAll) {
        ...
        if (result instanceof Error) {
          this.emit(
            constants.EVENT_FAIL,
            `"before all" hook in ${suite.title}: ${result.message}`
          );
          // suite执行结束
    ...
    if (suite._afterAll.length) {
      for (const fn of suite._afterAll) {
        ...
        if (result instanceof Error) {
          this.emit(
            constants.EVENT_FAIL,
            `"after all" hook in ${suite.title}: ${result.message}`
          );
          // suite执行结束
          this.emit(constants.EVENT_SUITE_END);
          return;
    ...
    if (_beforeEach.length) {
      for (const fn of _beforeEach) {
        ...
        if (result instanceof Error) {
          return this.emit(
            constants.EVENT_FAIL,
            `"before each" hook for ${test.title}: ${result.message}`
          );
        }
      }
    }
    ...
    if (_afterEach.length) {
      for (const fn of _afterEach) {
        ...
        if (result instanceof Error) {
          return this.emit(
            constants.EVENT_FAIL,
            `"after each" hook for ${test.title}: ${result.message}`
          );
        }
      }
    }
